...
as $$ declare b timestamptz; current_hits int; begin
  ...
  insert into public.api_rate_limits(key_hash,bucket,hits) values(encode(digest(raw_key,'sha256'),'hex'),b,1)
...
as $$ declare row_id uuid; begin
  ...
  values(p_device_code,encode(digest(p_device_secret,'sha256'),'hex'),'pending',now())
...
as $$ declare s public.screens; payload jsonb; begin
  ...
  if s.id is null or s.device_secret_hash is null or encode(digest(p_device_secret,'sha256'),'hex')<>s.device_secret_hash then raise exception 'unauthorized_device'; end if;
